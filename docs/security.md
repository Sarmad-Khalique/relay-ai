...
Relay assumes repository contents, repository configuration, model output, proposed commands, and provider
...
4. Relay restores that overlay before evaluating the diff and on resume after a crash.
...
6. macOS verification uses `sandbox-exec` to deny network; Linux uses Bubblewrap when installed. Relay
...
Prompts, source excerpts, diffs, and logs can remain sensitive even after token redaction. Relay stores them
locally with restrictive modes and telemetry is absent. Use `relay delete <run>` when retention is no longer
appropriate.
